          ...
          <div v-if="conversations.length">
            ...
            >
              <div class="conversation" @contextmenu.prevent.stop="e => showRightClickMenu(e,conversation)">
                <div class="avatar">
                  <img :src="baseUrl + '/api/files/'+conversation.data.avatar"/>
                  ...
                  <div class="conversation-bottom">
                    <div class="conversation-content" v-if="conversation.lastMessage.recalled">
                      <div v-if="conversation.type === 'private'">
                        {{ Number(conversation.lastMessage.senderId) === currentUser.id ? '你' : `"${conversation.data.nickname}"` }}撤回了一条消息
                      </div>
                      <div v-if="conversation.type === 'group'">
                        {{ Number(conversation.lastMessage.senderId) === currentUser.id ? '你' : `"${conversation.lastMessage.senderData.nickname}"` }}撤回了一条消息
                      </div>
                    </div>
                    <div class="conversation-content" v-else>
                      <div class="unread-text"
                           v-if="conversation.lastMessage.read === false && Number(conversation.lastMessage.senderId) === currentUser.id">
                        [未读]
                      </div>
                      <div v-if="conversation.type === 'private'">
                        {{ Number(conversation.lastMessage.senderId) === currentUser.id ? '我' : conversation.data.nickname }}:
                      </div>
                      <div v-else>
                        {{ Number(conversation.lastMessage.senderId) === currentUser.id ? '我' : conversation.lastMessage.senderData.nickname }}:
                      </div>
...
function chatLocation (conversation) {
  let path = '/CustomerService/chatOnline/conversations/privateChat/'+conversation.userId
  return {
    path: path,
    query: {
